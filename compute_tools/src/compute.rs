...
impl ComputeNode {
    ...
    pub fn prepare_pgdata(&self, compute_state: &ComputeState) -> Result<()> {
        ...
        let postgres_port = self.params.connstr.port().unwrap_or(55432);
        ...
        // OrioleDB recovery is NOT determined before basebackup.
        ...
        // OrioleDB serverless: configure Plan E and WAL replay recovery.
        //
        // On restart, OrioleDB metadata (control file, map files) is served
        // from PageServer via Plan E FPIs. B-tree data pages are read by
        // normal backends via PageServer GetPage. But in-memory state
        ...
        if matches!(spec.mode, ComputeMode::Primary) {
            ...
            // or inside shared_preload_libraries value.
            let has_orioledb = pg_conf_content.lines().any(|line| {
                let trimmed = line.trim();
                !trimmed.starts_with('#')
                    && (trimmed.starts_with("orioledb.")
                        || (trimmed.contains("shared_preload_libraries")
                            && trimmed.contains("orioledb")))
            });
        ...
        match spec.mode {
            ComputeMode::Primary => {
                // OrioleDB recovery is handled above via .orioledb_sync_lsn.
                // Don't trigger standby mode based on empty control file —
                // that's normal for first-time OrioleDB initialization.
            }
...
/// Patch SafeKeeper WAL files and copy them to pg_wal/ for OrioleDB recovery.
///
/// SafeKeeper WAL segments may have zero-filled pages before the active data
...
/// with valid XLogPageHeaders and copies the result to pgdata/pg_wal/.
fn patch_and_copy_wal_files(pgdata_path: &Path, sk_wal_dir: &Path) -> Result<()> {
    // PG17 WAL page magic
    ...
    // Read system identifier from pg_control (first 8 bytes of the file)
    let control_path = pgdata_path.join("global").join("pg_control");
    let control_data = std::fs::read(&control_path)
        .with_context(|| format!("reading pg_control at {}", control_path.display()))?;
    anyhow::ensure!(control_data.len() >= 8, "pg_control too small ({} bytes)", control_data.len());
    let sysid = u64::from_le_bytes(control_data[0..8].try_into().unwrap());
    info!("OrioleDB WAL patch: system_identifier = {}", sysid);

    if !sk_wal_dir.exists() {
        anyhow::bail!("SafeKeeper WAL dir does not exist: {}", sk_wal_dir.display());
    }

    let pg_wal_dir = pgdata_path.join("pg_wal");
    std::fs::create_dir_all(&pg_wal_dir)?;

    let mut file_count = 0u32;
    for entry in std::fs::read_dir(sk_wal_dir)? {
        let entry = entry?;
        let name = entry.file_name().to_string_lossy().to_string();

        // Match WAL segment files: 24 hex chars, optionally .partial
        let wal_name = name.trim_end_matches(".partial");
        if wal_name.len() != 24 || !wal_name.chars().all(|c| c.is_ascii_hexdigit()) {
            continue;
        }

        // Parse segment base LSN from filename: TTTTTTTTSSSSSSSSXXXXXXXX
        // T=timeline, S=log_hi (high 32 bits), X=segment number within log
        let timeline = u32::from_str_radix(&wal_name[0..8], 16)
            .with_context(|| format!("parsing timeline from WAL filename {}", wal_name))?;
        let log_hi = u64::from_str_radix(&wal_name[8..16], 16)
            .with_context(|| format!("parsing log_hi from WAL filename {}", wal_name))?;
        let seg_no = u64::from_str_radix(&wal_name[16..24], 16)
            .with_context(|| format!("parsing seg_no from WAL filename {}", wal_name))?;
        let seg_lsn: u64 = (log_hi << 32) | (seg_no * WAL_SEGMENT_SIZE);

        let mut data = std::fs::read(entry.path())?;
        let mut patched = 0u32;

        for page_off in (0..data.len()).step_by(WAL_PAGE_SIZE) {
            if page_off + WAL_PAGE_SIZE > data.len() {
                break;
            }

            // Skip pages that already have a valid magic number
            let magic = u16::from_le_bytes([data[page_off], data[page_off + 1]]);
            if magic != 0 {
                continue;
            }

            // Patch any page with zero magic — the header area (first 24/40 bytes)
            // is zero-filled by SafeKeeper. WAL record data may exist later
            // in the page (e.g., the first data page has records starting
            // mid-page but no header). We only write the header fields,
            // leaving existing record data untouched.

            let page_lsn = seg_lsn + page_off as u64;

            // Write xlp_magic (u16 LE, offset 0)
            data[page_off..page_off + 2].copy_from_slice(&XLOG_PAGE_MAGIC.to_le_bytes());
            // Write xlp_tli (u32 LE, offset 4)
            data[page_off + 4..page_off + 8].copy_from_slice(&timeline.to_le_bytes());
            // Write xlp_pageaddr (u64 LE, offset 8)
            data[page_off + 8..page_off + 16].copy_from_slice(&page_lsn.to_le_bytes());
            // xlp_info (offset 2) and xlp_rem_len (offset 16) stay zero

            if page_off == 0 {
                // First page of segment needs a long header with system info
                // xlp_info = XLP_LONG_HEADER (u16 LE, offset 2)
                data[page_off + 2..page_off + 4].copy_from_slice(&XLP_LONG_HEADER.to_le_bytes());
                // xlp_sysid (u64 LE, offset 24)
                data[page_off + 24..page_off + 32].copy_from_slice(&sysid.to_le_bytes());
                // xlp_seg_size (u32 LE, offset 32)
                data[page_off + 32..page_off + 36]
                    .copy_from_slice(&(WAL_SEGMENT_SIZE as u32).to_le_bytes());
                // xlp_xlog_blcksz (u32 LE, offset 36)
                data[page_off + 36..page_off + 40]
        ...
        let dest = pg_wal_dir.join(wal_name);
        std::fs::write(&dest, &data)?;
        info!(
            "OrioleDB WAL: {} → pg_wal/{} ({} zero-pages patched)",
            name, wal_name, patched
        );
        file_count += 1;
    }

    info!("OrioleDB WAL patch: {} segment files copied to pg_wal/", file_count);
    Ok(())
}
