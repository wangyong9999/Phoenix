...
impl DatadirModification<'_> {
    ...
    pub async fn ingest_batch(
        ...
        for meta in batch.metadata.iter() {
            ...
            // OrioleDB WAL keys are a non-rel relation-level stream (field1 ==
            // ORIOLEDB_WAL_KEY_PREFIX). They have no (rel, blkno) mapping, so
            // skip the relation-size bookkeeping for them.
            if key.field1 == pageserver_api::key::ORIOLEDB_WAL_KEY_PREFIX {
                continue;
            }
            let (rel, blkno) = key
                .to_rel_block()
    ...
    async fn put_rel_creation_v1(
        ...
        // Tolerate re-creation (idempotent) for OrioleDB synthetic relations
        // that emit SMGR_CREATE on every startup.
        if !rel_dir.rels.insert((rel.relnode, rel.forknum)) {
            // Already exists — skip silently instead of erroring.
            return Ok(());
        }
        if !dbdir_exists {
            self.pending_directory_entries
    ...
    async fn put_rel_creation_v2(
        ...
        if val == RelDirExists::Exists {
            // Already exists — tolerate re-creation (idempotent).
            return Ok(());
        }
        self.put(
            sparse_rel_dir_key,
