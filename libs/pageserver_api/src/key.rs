...
/// Create a key for an OrioleDB WAL stream entry.
/// OrioleDB WAL records are stored sequentially under this key space,
/// keyed by the record's full 64-bit LSN so that entries never collide
/// across the 4GB boundary. The LSN is split across field3/field6 so that
/// key ordering matches LSN ordering (field3 is the high 32 bits).
#[inline(always)]
pub fn orioledb_wal_key(lsn: u64) -> Key {
    Key {
        field1: ORIOLEDB_WAL_KEY_PREFIX,
        field2: 0,
        field3: (lsn >> 32) as u32,
        field4: 0,
        field5: 0,
        field6: lsn as u32,
    }
