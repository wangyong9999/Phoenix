				 ...
				 bool topxid_included)
{
	...
	if (likely(class == WALINSERT_NORMAL))
	{
		...
		{
			/* Relaxed: OrioleDB recovery replays from an older LSN
			 * while basebackup sets Insert->RedoRecPtr at the latest. */
...
StartupXLOG(void)
{
	...
	if (InRecovery)
	{
		...
		if (ArchiveRecoveryRequested && EnableHotStandby)
		{
			...
			/* OrioleDB serverless: basebackup at initdb_lsn may have
			 * invalid oldestActiveXID. Use FirstNormalTransactionId as fallback. */
			if (!TransactionIdIsValid(oldestActiveXID))
				oldestActiveXID = FirstNormalTransactionId;
...
PerformRecoveryXLogAction(void)
{
	...
	else if (IsOrioleDbRecoveryRequested())
	{
		/*
		 * OrioleDB serverless recovery: flush OrioleDB B-tree data via
		 * the checkpoint hook, but skip the PG checkpoint WAL record.
		 * Writing a checkpoint record would advance SafeKeeper LSN,
		 * causing a WAL flush gap on the next restart.
		 */
		if (CheckPoint_hook)
		{
			XLogRecPtr	curInsert = GetXLogInsertRecPtr();
			CheckPoint_hook(curInsert,
							CHECKPOINT_END_OF_RECOVERY | CHECKPOINT_IMMEDIATE);
		}
		else
			ereport(WARNING,
					(errmsg("OrioleDB recovery requested but CheckPoint_hook not registered"),
					 errhint("Ensure OrioleDB extension is loaded via shared_preload_libraries.")));
	}
	else
	{
