	...
	for (;;)
	{
		...
		while (prefetcher->next_block_id <= record->max_block_id)
		{
			...
			*lsn = record->lsn;
			...
			{
				if (IsOrioleDbRecoveryRequested())
				{
					/*
					 * During OrioleDB serverless recovery, WAL replay
					 * from an older LSN may reference blocks that don't
					 * exist at the current basebackup LSN. Safe to skip
					 * since non-OrioleDB records are filtered anyway.
					 */
					return LRQ_NEXT_NO_IO;
				}
				ereport(ERROR,
						(errmsg("could not prefetch relation %u/%u/%u block %u",
								reln->smgr_rlocator.locator.spcOid,
								reln->smgr_rlocator.locator.dbOid,
								reln->smgr_rlocator.locator.relNumber,
								block->blkno)));
