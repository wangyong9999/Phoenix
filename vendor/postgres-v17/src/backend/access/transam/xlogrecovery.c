...
static bool orioleDbRecoveryRequested = false; /* OrioleDB serverless WAL replay */
static XLogRecPtr orioleDbRedoStartLsn = InvalidXLogRecPtr; /* redo start for OrioleDB recovery */

bool
IsOrioleDbRecoveryRequested(void)
{
	return orioleDbRecoveryRequested;
}
				...
				bool *haveBackupLabel_ptr, bool *haveTblspcMap_ptr)
{
	...
	else if (NeonRecoveryRequested)
	{
		...
		else if (stat("orioledb_recovery.signal", &st) == 0)
		{
			/*
			 * OrioleDB serverless recovery: force crash recovery to replay
			 * OrioleDB WAL records from pg_wal/. The signal file contains
			 * the redo start LSN (where SafeKeeper WAL begins).
			 */
			char		lsnbuf[32];
			int			sigfd;
			ssize_t		nbytes;
			...
			sigfd = BasicOpenFile("orioledb_recovery.signal", O_RDONLY | PG_BINARY);
			if (sigfd >= 0)
			{
				nbytes = read(sigfd, lsnbuf, sizeof(lsnbuf) - 1);
				close(sigfd);
				if (nbytes > 0)
				{
					lsnbuf[nbytes] = '\0';
					if (sscanf(lsnbuf, "%X/%X", &hi, &lo) == 2)
					{
						orioleDbRedoStartLsn = ((uint64) hi) << 32 | lo;
						elog(LOG, "OrioleDB recovery: redo start LSN %X/%X",
							 hi, lo);
					}
				}
			}
			...
			wasShutdown = false;
			orioleDbRecoveryRequested = true;
			elog(LOG, "OrioleDB recovery: forcing crash recovery");
			unlink("orioledb_recovery.signal");
		}
		else
			wasShutdown = true;
	...
	if (checkPoint.redo < CheckPointLoc)
	{
		...
		InRecovery = true;
	else if (NeonRecoveryRequested && !wasShutdown)
	{
		/*
		 * OrioleDB serverless recovery: wasShutdown was set to false by
		 * orioledb_recovery.signal. Force crash recovery even though the
		 * control file says DB_SHUTDOWNED.
		 */
		InRecovery = true;
	}
...
FinishWalRecovery(void)
{
	...
	/*
	 * OrioleDB recovery: the redo replayed old WAL, so EndRecPtr is at an
	 * old position. But new WAL must start at the neon checkpoint LSN
	 * (where the basebackup's state is). Otherwise walproposer will see
	 * a gap between SafeKeeper's commit LSN and local WAL.
	 */
	/*
	 * Note: for OrioleDB serverless, endOfLog is NOT advanced. The Neon
	 * code path (NeonRecoveryRequested && endOfLog == RedoStartLSN at
	 * line 1771) handles initialization of the WAL page at the redo
	 * position. New WAL writing starts from there naturally.
	 */
...
PerformWalRecovery(void)
{
	...
	else
	{
		...
		if (NeonRecoveryRequested)
		{
			if (orioleDbRecoveryRequested &&
				XLogRecPtrIsValid(orioleDbRedoStartLsn))
			{
				/*
				 * OrioleDB serverless recovery: start reading WAL from
				 * the sync LSN where SafeKeeper WAL begins.
				 */
				elog(LOG, "OrioleDB recovery: redo from %X/%X",
					 LSN_FORMAT_ARGS(orioleDbRedoStartLsn));
				XLogPrefetcherBeginRead(xlogprefetcher, orioleDbRedoStartLsn);
				xlogreader->ReadRecPtr = orioleDbRedoStartLsn;
				RedoStartLSN = orioleDbRedoStartLsn;
			}
			else
	...
	if (record != NULL)
	{
		...
		/*
		 * OrioleDB serverless recovery: basebackup provides a consistent
		 * PG catalog at the latest LSN. Mark as consistent immediately so
		 * backends can connect after redo completes.
		 */
		if (orioleDbRecoveryRequested && !reachedConsistency)
		{
			/* Safe: OrioleDB recovery only runs on primaries.
			 * PG catalog is already consistent from the latest basebackup. */
			Assert(!StandbyModeRequested);
			reachedConsistency = true;
			ereport(LOG,
					(errmsg("OrioleDB recovery: marking consistent (basebackup is current)")));
		}
		...
		do
		{
			 ...
			 * Apply the record.
			 *
			 * During OrioleDB serverless recovery, only replay XACT
			 * (commit/abort) records to rebuild the CSN mapping.
			 * All other records are skipped:
			 *   - rmid=129 (OrioleDB B-tree): pages served by PageServer
			 *     via page-level delta WAL + wal-redo
			 *   - Other PG rmids: PG catalog already at latest state
			 *     from basebackup
			 */
			if (orioleDbRecoveryRequested)
			{
				if (record->xl_rmid == RM_XACT_ID && xact_redo_hook)
				{
					uint8 info = record->xl_info & XLOG_XACT_OPMASK;
					bool is_commit = (info == XLOG_XACT_COMMIT ||
									  info == XLOG_XACT_COMMIT_PREPARED);

					if (is_commit || info == XLOG_XACT_ABORT ||
						info == XLOG_XACT_ABORT_PREPARED)
					{
						xact_redo_hook(record->xl_xid,
									   xlogreader->EndRecPtr, is_commit);
					}
				}
				/* All other rmids (including 129) skipped */
			}
			else
