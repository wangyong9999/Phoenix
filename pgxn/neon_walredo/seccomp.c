...
seccomp_load_rules(PgSeccompRule *rules, int count)
{
	...
	/* Set a trap on getuid() to test seccomp bpf.
	 * Note: we use getuid instead of openat because OrioleDB's redo
	 * needs openat for file I/O in the wal-redo sandbox. */
	rule = PG_SCMP(getuid, SCMP_ACT_TRAP);
	if (do_seccomp_load_rules(&rule, 1, SCMP_ACT_ALLOW) != 0)
		ereport(FATAL,
				(errcode(ERRCODE_SYSTEM_ERROR),
				 errmsg("seccomp: could not load test trap")));

	/* Finally, check that getuid() now raises SIGSYS */
	(void) syscall(SCMP_SYS(getuid));
	if (!seccomp_test_sighandler_done)
		ereport(FATAL,
				(errcode(ERRCODE_SYSTEM_ERROR),
...
#define DIE_PREFIX "seccomp test signal handler: "
	...
	if (info->si_syscall != SCMP_SYS(getuid))
