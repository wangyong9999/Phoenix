...
#endif
{
	...
	{
		/*
		 * OrioleDB fallback: if page not in inmem buffer, try reading from
		 * OrioleDB's data files. During wal-redo, orioledb_redo() builds
		 * B-trees in OrioleDB's page pool which may evict to orioledb_data/.
		 */
		char		path[MAXPGPATH];
		int			fd;

		snprintf(path, sizeof(path), "orioledb_data/%u/%u",
				 reln->smgr_rlocator.locator.dbOid,
				 reln->smgr_rlocator.locator.relNumber);
		fd = open(path, O_RDONLY | PG_BINARY, 0);
		if (fd >= 0)
		{
			ssize_t		nbytes = pread(fd, buffer, BLCKSZ,
									   (off_t) blkno * BLCKSZ);
			close(fd);
			if (nbytes == BLCKSZ)
				return;  /* Found in OrioleDB data files */
		}
