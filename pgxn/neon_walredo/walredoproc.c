...
static PgSeccompRule allowed_syscalls[] =
{
	...
	/*
	 * OrioleDB: file I/O needed for B-tree page management during WAL redo.
	 * OrioleDB's orioledb_redo() creates directories and reads/writes
	 * B-tree pages to orioledb_data/ files during recovery.
	 */
	PG_SCMP_ALLOW(openat),
	PG_SCMP_ALLOW(close),
	PG_SCMP_ALLOW(fstat),
	PG_SCMP_ALLOW(newfstatat),
	PG_SCMP_ALLOW(mkdir),
	PG_SCMP_ALLOW(mkdirat),
	PG_SCMP_ALLOW(lseek),
	PG_SCMP_ALLOW(pread64),
	PG_SCMP_ALLOW(pwrite64),
	PG_SCMP_ALLOW(fsync),
	PG_SCMP_ALLOW(fdatasync),
	PG_SCMP_ALLOW(unlink),
	PG_SCMP_ALLOW(unlinkat),
	PG_SCMP_ALLOW(getdents64),
	PG_SCMP_ALLOW(access),
	PG_SCMP_ALLOW(faccessat2),
	PG_SCMP_ALLOW(fcntl),
	PG_SCMP_ALLOW(ftruncate),
	PG_SCMP_ALLOW(rename),
	PG_SCMP_ALLOW(renameat2),
...
WalRedoMain(int argc, char *argv[])
{
	...
	/*
	 * Load OrioleDB extension in light mode for page-level WAL redo.
	 * orioledb.so detects am_wal_redo_postgres and skips shmem/filesystem
	 * initialization, only registering RMGR 129 for orioledb_page_redo().
	 */
	load_file("$libdir/orioledb", false);
...
GetPage(StringInfo input_message)
{
	...
	/*
	 * OrioleDB: before reading the page, ensure OrioleDB's page pool has been
	 * flushed to disk. OrioleDB's redo builds B-trees in its own page pool,
	 * and pages may not be on disk yet. Call orioledb's evict if available.
	 *
	 * For now, we rely on the inmem_smgr fallback to read from orioledb_data/
	 * files. If pages are still in memory, they'll be written to disk by
	 * OrioleDB's checkpoint at the end of recovery.
	 */
