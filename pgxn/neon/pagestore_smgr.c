					  ...
					  neon_request_lsns *output, BlockNumber nblocks)
{
	...
	for (int i = 0; i < nblocks; i++)
	{
		...
		/*
		 * Under OrioleDB serverless recovery we read blocks from synthetic
		 * fake relations (Plan E FPI mirrors) that this compute has never
		 * tracked a last-written LSN for. Fall back to the request LSN so
		 * PageServer returns the latest image it has.
		 */
		/*
		 * OrioleDB fake relations (Plan E mirrors for map files, undo
		 * segments, xidmap, control file) are never registered in this
		 * compute's LwLsn table, so LwLsn stays InvalidXLogRecPtr even
		 * when the page genuinely exists in PageServer. This can also
		 * happen for a regular relation on the first read after a
		 * stateless restart before xlogrecovery sets flush/redo pointers.
		 *
		 * Rather than crashing on the diagnostic assertion, fall back to
		 * the best LSN we can produce (flush → redo → UINT64_MAX). The
		 * Neon wire protocol treats UINT64_MAX as "latest" — PageServer
		 * returns the most recent image it has for the key, which is
		 * exactly what we need here.
		 */
		if (last_written_lsns[i] == InvalidXLogRecPtr)
		{
			last_written_lsns[i] = GetFlushRecPtr(NULL);
			if (last_written_lsns[i] == InvalidXLogRecPtr)
				last_written_lsns[i] = GetRedoRecPtr();
			if (last_written_lsns[i] == InvalidXLogRecPtr)
				last_written_lsns[i] = (XLogRecPtr) UINT64_MAX;
		}
...
#if PG_VERSION_NUM >= 150000
				/*
				 * UINT64_MAX is the sentinel our OrioleDB-serverless fallback
				 * uses when the compute has no LwLsn tracking for a fake
				 * relation. PageServer treats it as "latest"; the diagnostic
				 * assertion below was not written with that in mind, so skip
				 * it in that case.
				 */
				Assert(last_written_lsn == GetCurrentReplayRecPtr(NULL)
					   || IsOrioleDbRecoveryRequested()
					   || last_written_lsn == (XLogRecPtr) UINT64_MAX);
#endif
