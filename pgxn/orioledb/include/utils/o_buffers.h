...
typedef struct
{
	...
	uint32		buffersCount;
	/*
	 * Plan B (Neon serverless): stable per-stream id used to form a synthetic
	 * relNumber for the PageServer FPI mirror. Each OBuffersDesc must pick a
	 * unique small integer; the mapping into relNumber is owned by o_buffers.c.
	 * 0 means "not stable / opt out of Plan B mirroring".
	 */
	uint16		planBLogId;
