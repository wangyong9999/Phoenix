...
orioledb_redo(XLogReaderState *record)
{
	 ...
	 * by wal_decoder as Value::Image).
	 */
	if (IsOrioleDBPageLevelWal(XLogRecGetInfo(record)))
	{
		orioledb_page_redo(record);
		return;
	}

	/* Skip non-container OrioleDB records (checkpoint control, page images) */
							 ...
							 int worker_id, bool sync)
{
	...
	if (!COMMITSEQNO_IS_ABORTED(csn) && sync)
	{
		...
		/*
		 * In Neon serverless recovery, skip undo flush and checkpoint stack
		 * walks — they write WAL which causes SafeKeeper LSN desync on
		 * subsequent restarts. The data is rebuilt from WAL anyway.
		 */
