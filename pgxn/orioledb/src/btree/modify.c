						   ...
						   &haveRedundantRowLocks, &context->lockStatus))
	{
		...
		else
		{
			...
			if (COMMITSEQNO_IS_ABORTED(csn))
			{
				...
				/* Page-level WAL: undo rollback — emit FPI */
				orioledb_page_wal_emit_fpi(desc, blkno, ORIOLEDB_XLOG_UNDO_APPLY);
				END_CRIT_SECTION();
			}
...
o_btree_modify_delete(BTreeModifyInternalContext *context)
{
	...
	/* Page-level WAL: emit delete delta for Neon PageServer */
	orioledb_page_wal_leaf_delete(desc, blkno,
								  BTREE_PAGE_LOCATOR_GET_OFFSET(page, &loc),
								  tuphdr);
...
o_btree_modify_lock(BTreeModifyInternalContext *context)
{
	...
	/* Page-level WAL: emit lock delta for Neon PageServer */
	orioledb_page_wal_leaf_delete(desc, blkno,
								  BTREE_PAGE_LOCATOR_GET_OFFSET(page, &loc),
								  tuphdr);
