								 ...
								 int tupleWaitersCount)
{
		...
		BTREE_PAGE_FREE_SPACE(p))
	{
		...
		for (i = 0; i <= tupleWaitersCount; i++)
		{
			...
			/* Page-level WAL: emit delta for Neon PageServer */
			orioledb_page_wal_leaf_insert(desc, blkno,
										  BTREE_PAGE_LOCATOR_GET_OFFSET(p, &loc),
										  &tuphdr,
										  (Pointer) tuple.data, tuplen);
	...
	if (!split)
	{
		...
		/* Page-level WAL: compaction reorganizes entire page — emit FPI */
		orioledb_page_wal_emit_fpi(desc, blkno, ORIOLEDB_XLOG_COMPACT);
							   ...
							   int reserve_kind)
{
	...
	if (fit == BTreeItemPageFitAsIs)
	{
		...
		if (O_PAGE_IS(p, LEAF))
		{
			if (!insert_item->replace)
			{
				orioledb_page_wal_leaf_insert(
					desc, blkno,
					BTREE_PAGE_LOCATOR_GET_OFFSET(p, &loc),
					(BTreeLeafTuphdr *) insert_item->tupheader,
					(Pointer) insert_item->tuple.data,
					insert_item->tuplen);
			}
			else
			{
				orioledb_page_wal_leaf_update(
					desc, blkno,
					BTREE_PAGE_LOCATOR_GET_OFFSET(p, &loc),
					(BTreeLeafTuphdr *) insert_item->tupheader,
					(Pointer) insert_item->tuple.data,
