...
/*
 * Neon Serverless adaptation layer.
 *
 * When running on Neon, route OrioleDB's B-tree page I/O through
 * PG's smgr interface so that Neon's smgr_hook intercepts and
 * redirects to PageServer.
 *
 * OrioleDB's (datoid, relnode, segno, offset) maps to PG's
 * (spcOid=DEFAULTTABLESPACE_OID, dbOid=datoid, relNumber=relnode,
 *  forkNumber=MAIN_FORKNUM, blockNumber=global_offset/BLCKSZ)
 */
static int neon_io_checked = 0;
static bool neon_io_enabled = false;

static void
orioledb_neon_io_check(void)
{
	if (neon_io_checked)
		return;
	neon_io_checked = 1;
...
/*
 * orioledb_planE_materialize - on-demand rehydration of a local file from
 * PageServer-hosted Plan E FPIs.
 *
 * OrioleDB assumes the file system is the persistent substrate. Under Neon's
 * stateless-compute model the local disk is transient, so on a fresh
 * compute (or after cache eviction) many files don't exist locally even
 * though their content is authoritative in PageServer via Plan E FPIs.
 *
 * This helper pulls every block of the synthetic relation
 *   (spc=DEFAULT, db=datoid, rel=relnode, fork=forknum)
 * from PageServer and materialises them at `local_path`. After the call,
 * OrioleDB's normal file-system code paths open and read it as if it had
 * always been there.
 *
 * Uses smgrexists + smgrnblocks preflight rather than PG_TRY/PG_CATCH so
 * caller-held LWLocks remain balanced if the Neon smgr raises. Safe to call
 * from both backend and (post-recovery) startup contexts.
 *
 * Returns true when the file was fully materialised, false otherwise (caller
 * should fall back to its pre-existing behaviour — typically "treat as empty").
 */
bool
orioledb_planE_materialize(const char *local_path, Oid datoid, uint32 relnode,
						   ForkNumber forknum)
{
	...
	dst = PathNameOpenFile(local_path,
	...
	{
		smgrread(reln, forknum, blk, page);
		if (OFileWrite(dst, page, BLCKSZ, (off_t) blk * BLCKSZ,
	...
	FileSync(dst, WAIT_EVENT_DATA_FILE_SYNC);
	FileClose(dst);

	elog(DEBUG1,
		 "OrioleDB Plan E: materialised %s from PageServer (%u blocks)",
		 local_path, nblocks);
	return true;
}

/*
 * Plan E: On page eviction, emit a WAL record containing the full page image
 * with a proper block reference. This lets Neon's PageServer receive and index
 * the page via the standard WAL pipeline.
 *
 * The page image is the FINAL result after all B-tree operations (inserts,
 * splits, merges). No B-tree replay needed on the PageServer side — it just
 * stores and serves the image.
 *
 * We use a special ORIOLEDB_XLOG_PAGE_IMAGE info byte to distinguish these
 * WAL records from the row-level ORIOLEDB_XLOG_CONTAINER records.
 */
/* Use ORIOLEDB_XLOG_PAGE_IMAGE from orioledb.h */
...
btree_open_smgr(BTreeDescr *descr)
{
	...
	else
	{
		...
		/*
		 * In Neon serverless mode, skip opening the local segment file.
		 * Data will be served from PageServer via btree_smgr_read's Neon
		 * path (smgrread → GetPage). The local file may not exist on a
		 * stateless compute restart.
		 */
		orioledb_neon_io_check();
		if (!neon_io_enabled)
				 ...
				 int amount, off_t offset)
{
	...
	/* Neon detection (checked once per backend) */
	orioledb_neon_io_check();
				...
				int amount, off_t offset)
{
	...
	/*
	 * Neon Plan E: when running serverless, read OrioleDB B-tree pages
	 * from PageServer via PG smgr (which Neon hooks to neon_smgr).
	 * PageServer has these pages as FPIs emitted during checkpoint
	 * by write_page_to_disk → XLogRegisterBlock(REGBUF_FORCE_IMAGE).
	 *
		...
		&& !AmStartupProcess())
	{
		...
		/*
		 * Try smgrexists first. For OrioleDB's synthetic relations
		 * (never created via DDL), this returns false if PageServer
		 * has no block-level data yet. If true, use smgrnblocks and
		 * smgrread normally. If false, fall through to local path.
		 *
		 * NOTE: PG_TRY/PG_CATCH is NOT safe here because callers
		 * (sys_tree_init) may hold LWLocks. FlushErrorState
		 * corrupts holdoff counts and memory contexts.
		 */
		if (smgrexists(reln, MAIN_FORKNUM))
		{
			BlockNumber nblocks = smgrnblocks(reln, MAIN_FORKNUM);

			while (read < amount)
			{
				int			step = Min(ORIOLEDB_BLCKSZ, amount - read);

				blkno = (offset + read) / ORIOLEDB_BLCKSZ;
				if (blkno < nblocks)
				{
					if (step == ORIOLEDB_BLCKSZ)
						smgrread(reln, MAIN_FORKNUM, blkno, buffer + read);
					else
					{
						char	tmpbuf[BLCKSZ];
						smgrread(reln, MAIN_FORKNUM, blkno, tmpbuf);
						memcpy(buffer + read,
							   tmpbuf + ((offset + read) % ORIOLEDB_BLCKSZ),
							   step);
					}
				}
				else
					memset(buffer + read, 0, step);
				read += step;
			}
			return read;
		}
		/*
		 * smgrexists returned false — synthetic relation has no data in
		 * PageServer yet. Fall through to local file path.
		 */
	}
				   ...
				   Pointer page, off_t page_size)
{
	...
	if (!OCompressIsValid(desc->compress))
	{
		...
		/*
		 * Plan E: emit FPI to PageServer alongside the local write.
		 *
		 * We also fire during the startup process's end-of-recovery checkpoint
		 * so that block 0 of every persistent tree gets an image in PageServer
		 * — otherwise a subsequent stateless restart cannot find the root
		 * page and falls back to a zero page (FATAL "Page version 0").
