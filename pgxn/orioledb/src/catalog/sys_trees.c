...
/*
 * Reset all system tree initialized flags for Neon deferred control
 * file loading. Called from checkpointable_tree_init when control file
 * is loaded from PageServer after shmem_startup initialized them with
 * chkp_num=0.
 */
void
sys_trees_reset_initialized(void)
{
	int			i;

	for (i = 0; i < SYS_TREES_NUM; i++)
	{
		sysTreesShmemHeaders[i].initialized = false;
		sysTreesShmemHeaders[i].rootInfo.rootPageBlkno = OInvalidInMemoryBlkno;
		sysTreesShmemHeaders[i].rootInfo.metaPageBlkno = OInvalidInMemoryBlkno;
		sysTreesShmemHeaders[i].rootInfo.rootPageChangeCount = 0;
		sysTreesDescrs[i].initialized = false;
	}
}
