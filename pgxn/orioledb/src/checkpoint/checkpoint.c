...
/* True during shutdown checkpoint — guards FPI emission */
bool		checkpoint_is_shutdown = false;
...
o_perform_checkpoint(XLogRecPtr redo_pos, int flags)
{
	 ...
	 * after the final checkpoint record, causing PANIC.
	 */
	checkpoint_is_shutdown = ((flags & CHECKPOINT_IS_SHUTDOWN) != 0);

	/*
	 * In Neon mode, register the control file's synthetic relation with
	 * PageServer via XLOG_SMGR_CREATE. Only on non-shutdown checkpoints.
	 */
	...
	/*
	 * Neon Log-is-Data: flush the entire xidBuffer to xidmap before
	 * completing the checkpoint. This ensures ALL committed transaction
	 * CSN mappings are persisted to xidmap files, which are mirrored
	 * to PageServer via Plan B FPIs. After this, a stateless restart
	 * can read all CSN data from PageServer without WAL replay.
	 */
	if (smgr_hook != NULL && !checkpoint_is_shutdown)
	{
		OXid nextXid = pg_atomic_read_u64(&xid_meta->nextXid);
		OXid globalXmin = pg_atomic_read_u64(&xid_meta->globalXmin);

		if (nextXid > globalXmin)
			fsync_xidmap_range(globalXmin, nextXid,
...
o_after_checkpoint_cleanup_hook(XLogRecPtr checkPointRedo, int flags)
{
	...
	*was_in_recovery = flags == 0;
	...
	/*
	 * Neon serverless: update .orioledb_sync_lsn after each checkpoint.
	 * This bounds the WAL replay window on next restart to only the
	 * post-checkpoint delta. The FPIs emitted during checkpoint are in
	 * PageServer, so GetPage provides checkpoint-consistent base pages.
	 */
	if (NeonRecoveryRequested && !(flags & CHECKPOINT_END_OF_RECOVERY))
	{
		char		lsnbuf[64];
		char		path[MAXPGPATH];
		char		*datadir;
		...
		snprintf(path, sizeof(path), "%s/../.orioledb_sync_lsn", datadir);
		snprintf(lsnbuf, sizeof(lsnbuf), "%X/%X",
		...
		if (fd >= 0)
		{
			(void) write(fd, lsnbuf, strlen(lsnbuf));
			close(fd);
			elog(LOG, "OrioleDB: sync_lsn updated to %s after checkpoint %u",
				 lsnbuf, chkpNum);
		}
	}
...
checkpoint_ix(int flags, BTreeDescr *descr)
{
	...
	/*
	 * Plan E: mirror the ENTIRE map file to PageServer (block 0 is the
	 * header we just wrote, subsequent blocks hold the free-extent list).
	...
	if (smgr_hook != NULL && !checkpoint_is_shutdown && XLogInsertAllowed())
	{
		...
		rlocator.dbOid = datoid;
		rlocator.relNumber = relnode;

		file_size = FileSize(file);
		for (emit_offset = 0; emit_offset < file_size; emit_offset += BLCKSZ)
		{
			int			got;

			memset(page, 0, BLCKSZ);
			got = (int) Min((off_t) BLCKSZ, file_size - emit_offset);
			if (OFileRead(file, page, got, emit_offset,
						  WAIT_EVENT_SLRU_READ) != got)
			{
				elog(WARNING, "OrioleDB: map FPI read failed at offset %lu "
					 "for (%u,%u) chkp=%u — remaining %lu bytes not emitted",
					 (unsigned long) emit_offset, datoid, relnode, chkpNum,
					 (unsigned long) (file_size - emit_offset));
				break;
			}

			XLogBeginInsert();
			XLogRegisterBlock(0, &rlocator, INIT_FORKNUM,
							  (BlockNumber) (emit_offset / BLCKSZ),
							  page, REGBUF_FORCE_IMAGE | REGBUF_WILL_INIT);
			XLogInsert(ORIOLEDB_RMGR_ID, ORIOLEDB_XLOG_PAGE_IMAGE);
		}
		elog(DEBUG1, "OrioleDB: map FPI for (%u,%u) chkp=%u (%lu bytes, %lu emitted)",
			 datoid, relnode, chkpNum, (unsigned long) file_size,
			 (unsigned long) emit_offset);
	}
						 ...
						 uint32 *map_chkp_num, bool clear_tree)
{
	...
	*evicted_data = read_evicted_data(desc->oids.datoid,
	...
	else
	{
		...
		{
			/*
			 * Plan E map-file fallback. Safe in any process context because we
			 * use smgrexists / smgrnblocks (cheap metadata reads that do not
			 * raise ERROR under normal conditions) rather than PG_TRY/PG_CATCH
			 * that would unbalance LWLock state held by sys_tree_init_if_needed.
			 * Sys trees must go through this path too — otherwise startup
			 * initialises them as empty and later backends see empty catalogs.
			 */
			if (smgr_hook != NULL)
			{
				RelFileLocator rlocator;
				...
				{
					bool exists = smgrexists(reln, INIT_FORKNUM);
					elog(LOG, "evictable_tree_init_meta: (%u,%u) INIT fork "
						 "smgrexists=%d", desc->oids.datoid,
						 desc->oids.relnode, exists);
					if (exists)
					{
						nblocks = smgrnblocks(reln, INIT_FORKNUM);
						have_map = (nblocks > 0);
						elog(LOG, "evictable_tree_init_meta: nblocks=%u "
							 "have_map=%d", nblocks, have_map);
					}
				}
				...
				{
					/*
					 * Pull the full map file back to local disk from the
					 * PageServer mirror, then continue through the existing
					 * local-file code path (seq_buf etc). This keeps the
					 * rest of OrioleDB blissfully unaware of Plan E.
					 */
					File		dst;
					BlockNumber blk;
					char		page[BLCKSZ];

					(void) MakePGDirectory("orioledb_data");
					/* Ensure directory exists for stateless restart */
					{
						char dirpath[MAXPGPATH];
						snprintf(dirpath, sizeof(dirpath), "orioledb_data/%u", desc->oids.datoid);
						if (MakePGDirectory(dirpath) < 0 && errno != EEXIST)
							elog(LOG, "mkdir %s failed: %m", dirpath);
					}
					dst = PathNameOpenFile(prev_chkp_fname,
										   O_CREAT | O_RDWR | O_TRUNC | PG_BINARY);
					if (dst >= 0)
					{
						bool rehydrate_ok = true;

						for (blk = 0; blk < nblocks; blk++)
						{
							smgrread(reln, INIT_FORKNUM, blk, page);
							if (OFileWrite(dst, page, BLCKSZ,
										   (off_t) blk * BLCKSZ,
										   WAIT_EVENT_SLRU_WRITE) != BLCKSZ)
							{
								elog(WARNING,
									 "evictable_tree_init_meta: write failed "
									 "at block %u of (%u,%u) — cleaning up",
									 blk, desc->oids.datoid,
									 desc->oids.relnode);
								rehydrate_ok = false;
								break;
							}
						}
						FileSync(dst, WAIT_EVENT_SLRU_SYNC);
						FileClose(dst);

						if (!rehydrate_ok)
						{
							unlink(prev_chkp_fname);
							/* Fall through to empty-tree default below */
						}
						else
						{
						/* Re-open for reading and pick up the header */
						prev_chkp_file = PathNameOpenFile(prev_chkp_fname,
														  O_RDONLY | PG_BINARY);
						if (prev_chkp_file >= 0 &&
							OFileRead(prev_chkp_file, (Pointer) &file_header,
		...
		else					/* if checkpoint file exist */
		{
			*map_chkp_num = prev_chkp_tag.num;
			...
			{
				/*
				 * Neon serverless: basebackup may create empty stub files.
				 * Treat as "file not found" — initialize empty tree.
				 * WAL recovery will rebuild the data.
				 */
				elog(LOG, "evictable_tree_init_meta: could not read header "
					 "from %s — initializing empty tree (WAL recovery will rebuild)",
					 prev_chkp_fname);
				file_header.rootDownlink = InvalidDiskDownlink;
				file_header.datafileLength = 0;
				file_header.numFreeBlocks = 0;
...
checkpointable_tree_init(BTreeDescr *desc, bool init_shmem, bool *was_evicted)
{
	...
	/*
	 * In Neon mode, register synthetic relations with PageServer
	 * via XLOG_SMGR_CREATE on first init only. Must NOT run during
	 * shutdown checkpoint (causes PANIC: concurrent WAL activity).
	 */
	if (init_shmem && desc->storageType == BTreeStoragePersistence)
		orioledb_page_wal_register_tree(desc);
