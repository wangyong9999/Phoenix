...
| **R13** (new) | `test_e2e_crud.sh` clean-shutdown + restart 路径下 `SELECT count(*)` 返回 0（pre-restart count=4000） | checkpoint skip + chkpNum 错位 | Open | 根因初步定位：第一 session 内 `CHECKPOINT;` 在 chkpNum=2 时正确捕获了 (5,16476) 的数据（root downlink off=70, itemsCount=35, 35 leaf pages, datafileLength=71 均已被 `checkpoint_map_write_header` 写入 PageServer）。第二 session（stateless restart）的 end-of-recovery checkpoint chkpNum=3 也正确跑完；startup process (PID 720064) 的 `evictable_tree_init_meta (5, 16476)` 读取 map 后也显示了正确的 itemsCount=35 level=1。但后续 backend (PID 720094) 在 `checkpointable_tree_init` 时用 `chkp_num=3`（startup 用 2），导致 map file 读取串错了 chkpNum 槽，访问落到空 root。建议修法方向：signal-path 下，所有 BTreeStoragePersistence 树在 end-of-recovery checkpoint 都必须强制 `dirtyFlag=true` 且 `checkpoint_map_write_header` 必须写入 PageServer（即使 in-memory 副本无变化），以保证 chkpNum 所有 slot 的 map file 在 PageServer 侧单调一致。需结合 Phase 3 推进（彻底去 signal path 即可规避此 chkpNum 槽切换问题）。|
